...
class OdooSearch(http.Controller):
	...
	def fetch_query_data(self, **kwargs):
		...
		for field in ['name', 'origin', 'email', 'phone', 'mobile', 'e-mail', 'emailid', 'zip', 'street', 'website', 'description', 'desc', 'short_desc', 'price'] :
			...
			for many2one_field in many2one_fields:
				...
				if 'report' in DB_MODEL.model:
					continue
				if 'model' in DB_MODEL.model:
					continue
				if 'website' in DB_MODEL.model:
					continue
				if 'action' in DB_MODEL.model:
					continue
				if 'base.' in DB_MODEL.model:
					continue
				if 'ir.' in DB_MODEL.model:
					continue
				if 'theme' in DB_MODEL.model:
					continue
				if 'ir.module.module' in DB_MODEL.model:
					continue
